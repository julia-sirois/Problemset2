...
You should have used `filter(Halloween, Type == "Not scary" & age <= 10)`. Because you used `|`, you have some scary costumes and some NA ages in `notscarykids`
...
ggplot(Halloween, aes(x = Costume, y = age)) + coord_flip() +
  ...
  theme_bw()
```
The `theme_bw()` isn't doing anything.
